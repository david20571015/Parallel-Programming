...
int main(int argc, char **argv) {
  ...
  int opt;
  static struct option long_options[] = {{"filter", 1, 0, 'f'},
                                         {"input", 1, 0, 'i'},
                                         {"help", 0, 0, '?'},
                                         {0, 0, 0, 0}};
  ...
  // cuda
  double minCuda = 0;
  double recordCuda[10] = {0};
  for (int i = 0; i < 10; ++i) {
    memset(outputImage, 0, dataSize);
    start_time = currentSeconds();
    // Run the host to execute the kernel
    hostFE(filterWidth, filter, imageHeight, imageWidth, inputImage,
               outputImage);
    end_time = currentSeconds();
    recordCuda[i] = end_time - start_time;
  }
  qsort(recordCuda, 10, sizeof(double), compare);
  for (int i = 3; i < 7; ++i) {
    minCuda += recordCuda[i];
  }
  minCuda /= 4;
  ...
  for (int i = 0; i < 10; ++i) {
    memset(refImage, 0, dataSize);
    start_time = currentSeconds();
    serialConv(filterWidth, filter, imageHeight, imageWidth, inputImage,
               refImage);
    end_time = currentSeconds();
  ...
  for (i = 0; i < imageHeight; i++) {
    for (j = 0; j < imageWidth; j++) {
      if (abs(outputImage[i * imageWidth + j] - refImage[i * imageWidth + j]) >
          10) {
        diff_counter += 1;
      }
    }
